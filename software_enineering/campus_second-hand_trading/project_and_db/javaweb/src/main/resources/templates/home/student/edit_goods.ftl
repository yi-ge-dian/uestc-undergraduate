...
<head>
    ...
    <title>${siteName!""}交易平台--修改物品</title>
</head>
<body>
<br><br><br><br>
<!-- section -->
        ...
        <div class="row">
            <div class="col-12 col-md-7 col-xl-8">
                <div class="row">
                    <!-- section title -->
                    <div class="col-12">
                        <h2 class="section__title">修改商品</h2>
                    </div>
                    <!-- end section title -->
                    <div class="col-12">
                        <form action="edit_goods" class="form form--contacts" method="post" id="publish-form">
                            <input type="hidden"  name="id" value="${goods.id}" >
                            <input type="hidden" id="photo" name="photo" value="${goods.photo}" class="required" tips="请上传图片">
                            <div id="show-img" style="">
                                <h2 style="color: #7A56D4">已上传图片</h2>
                                <br>
                                <img id="uploaded-img" src="/photo/view?filename=${goods.photo}" alt="" width="80px" height="80px">
                            </div>
                            <br>
                            ...
                            <input type="text" class="form__input required" id="name" name="name" placeholder="最多18个字"
                                    value="${goods.name}"maxlength="18" tips="请填写商品名称">
                            <h2 style="color: #7A56D4">购入价格</h2>
                            <input type="number" class="form__input required"  value="${goods.buyPrice}" placeholder="￥" id="buyPrice" name="buyPrice" tips="请填写购入价格且只能为数字">
                            <h2 style="color: #7A56D4">出售价格</h2>
                            <input type="number" class="form__input required" value="${goods.sellPrice}" placeholder="￥" id="sellPrice" name="sellPrice" tips="请填写出售价格且只能为数字">
                            ...
                            <select name="" class="form__input" id="cid">
                                ...
                                <#list goodsCategorys as goodsCategory>
                                    ...
                                    <#if goods.goodsCategory.parent.id == goodsCategory.id>
                                    selected
                                    </#if>
                            ...
                            <select name="goodsCategory.id" class="form__input" id="cid2" name="goodCategory.id">
                                ...
                                <#if goodsCategory.parent??>
                                    <#if goodsCategory.parent.id == goods.goodsCategory.parent.id>
                                        ...
                                        <#if goods.goodsCategory.id == goodsCategory.id>
                                        selected
                                        </#if>
                                        >${goodsCategory.name}</option>
                                    <#else>
                            ...
                            <textarea name="content" id="desc" class="form__textarea required" placeholder="简要介绍下你的商品，建议填写商品用途、新旧程度、原价等信息，商品详情至少十五个字" tips="请填写商品详情">${goods.content}</textarea>
        ...
        $("#submit-btn").click(function () {
            ...
            if (flag){
                ...
                ajaxRequest('edit_goods','post',$("#publish-form").serialize(),function () {
                    showConfirmSuccessMsg_publish("你已成功修改商品")
                })
            }
        });
        //监听图片上传按钮
