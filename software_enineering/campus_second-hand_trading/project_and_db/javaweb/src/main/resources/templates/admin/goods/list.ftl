                  ...
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td style="vertical-align:middle;">${goods.name}</td>
                        <td style="vertical-align:middle;">
                        ...
                        <td style="vertical-align:middle;">
                          <#if goods.recommend == 0>
                            <font style="color:limegreen">等待审核</font>
                          <#elseif goods.recommend == 1>
...
function recommend(url){
  ...
  var id = $("input[type='checkbox']:checked").val();
  ajaxRequest('recommend','post',{id:id,recommend:2},function(){
    showConfirmSuccessMsg('审核成功');
  });
}
//不推荐
function unrecommend(url){
  if($("input[type='checkbox']:checked").length != 1){
    showWarningMsg('请选择一条数据');
    return;
  }
  var id = $("input[type='checkbox']:checked").val();
  ajaxRequest('recommend','post',{id:id,recommend:1},function(){
    showConfirmSuccessMsg('审核不通过');
  });
}
//调用删除方法
